...
- **`game-performance-booster`** (`.claude/agents/game-performance-booster.md`) — given a `game_id`, audits that game's engine (`components/games/<id>/engine.ts`) for the per-frame draw anti-patterns SPEC 14 (`specs/14-frogger-performance.md`) fixed in Frogger — per-entity `ctx.shadowBlur`/`shadowColor`, redrawing cacheable shapes, gradients/allocations/O(n) `.find` lookups in the hot loop, missing sprite-cache invalidation on `resize()`/`setSkin()` — and **implements the fix** (the sprite pre-render + `spriteCache`/`getOrCreateSprite` pattern), verifying with `npx tsc --noEmit`. Unlike the four auditors above, this one edits files (tools include `Edit`, `Write`, `Bash`). Stays within SPEC 14's scope: no visual, gameplay, or engine-contract changes; one game at a time.
